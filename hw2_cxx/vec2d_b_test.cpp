...
// prototype for Gram-Schmidt2d_b routine
...
int main(int argc, char* argv[]) {
  ...
  a.Write();
  std::cout << "writing array of 0.1, 0.2,...., 1.9, 2.0 :" << std::endl;
  b.Write();
  std::cout << "writing array of 1, 2,.....,19, 20 :" << std::endl;
  c.Write();
  ...
  std::cout << "Testing vector copy, should be 1, 2,..., 19, 20 " << std::endl;
  a.Copy(c);
  a.Write();
  ...
  vec2d_b X[] = {vec2d_b(10,2), vec2d_b(10,2), vec2d_b(10,2), vec2d_b(10,2), vec2d_b(10,2)};
  ...
  vec2d_b d = Linspace(0.0, 19.0, 5, 4);
  d.Write();

  std::cout << "Testing Random" << std::endl;
  vec2d_b f = Random(5,4);
  f.Write();
  ...
  for (int i=0; i<6; i++) {
    ...
    std::cout << "Running GramSchmidt2d_b process with (" << m <<", "<< n <<")" << std::endl;
